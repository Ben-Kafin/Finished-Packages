...
class vasp_ae_wfc(object):
    ...
    def get_beta_njk(self, Cg):
        '''
        For SOC/NCL, Cg is a full spinor with 2*nplw coefficients.
        The PAW projectors are spin-independent, so we project each
        spinor component separately and concatenate: [beta_up, beta_dn].
        '''
        if self._pswfc._lsoc:
            nplw = self._q_proj.nplw
            beta_up = self._q_proj.proj(Cg[:nplw])
            beta_dn = self._q_proj.proj(Cg[nplw:])
            self._beta_njk = np.concatenate([beta_up, beta_dn])
        else:
