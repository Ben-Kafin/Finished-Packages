...
class Unified_STM_Simulator:
    ...
    def _converge_tip_height(self, z_map_gpu, grid_xy_gpu, emin, emax, target_ldos,
        ...
        for i in range(max_iter):
            t0 = time()
            ld_1, ld_2, a_grid = self._calculate_ldos_at_points_gpu(
                cp.hstack([grid_xy_gpu, z_map_gpu[:, None]]), emin, emax,
                use_energy_decay=use_decay, preserve_orbitals=False)
            # Topography always uses total LDOS
            if self.spin_mode == SpinMode.COLLINEAR_POL:
                cur_ldos = gpu_simpson(ld_1 + ld_2, a_grid)
            else:
...
class Interactive_STM_Simulator(Unified_STM_Simulator):
    ...
    def run_interactive(self, grid_res=64, topo_bias=0.2, topo_height=2.5,
        ...
        cache_name = f"global_topo_{topo_bias}V_{topo_height}A_{grid_res}px{self._broadening_cache_suffix()}.npy"
        if exists(cache_name):
            ...
            ld_1, ld_2, init_engs = self._calculate_ldos_at_points_gpu(
                cp.hstack([grid_xy_gpu, z_fixed[:, None]]), t_emin, t_emax,
                use_energy_decay=self.use_decay_topo, preserve_orbitals=False)
            topo_total = self._combine_ldos_total(ld_1, ld_2)
            target_setp = cp.max(gpu_simpson(topo_total, init_engs))
    ...
    def _build_ui(self):
        ...
        self.chk = CheckButtons(plt.axes([0.08, 0.02, 0.12, 0.08]),
                                ['Atoms', 'Decay', 'Norm', 'Mag', 'Cell', 'Decomp', 'Dcmp Norm'],
                                [self.show_atoms, self.use_decay_ldos, self.normalize, self.show_mag,
                                 self.show_unit_cell, self.show_decomp, self.show_dcmp_norm])
    ...
    def _recompute_global_topo(self):
        ...
        cache_name = f"global_topo_{self.global_topo_bias}V_{self.topo_height}A_{grid_res}px{self._broadening_cache_suffix()}.npy"
        if exists(cache_name):
            print(f"[*] Loading Cached Global Topography: {cache_name}")
            self.current_z_map = np.load(cache_name)
        else:
    ...
    def _update_all(self, full_refresh=False):
        ...
        if needs_topo:
            if self.mode == 'Line':
                ...
                ld_1, ld_2, l_engs = self._calculate_ldos_at_points_gpu(
                    cp.hstack([p_xy_gpu, cp.full((self.npts, 1), self.z_highest_atom + self.ldos_height, dtype=cp.float32)]),
                    l_emin, l_emax, use_energy_decay=self.use_decay_topo, preserve_orbitals=False)
                target = cp.max(gpu_simpson(self._combine_ldos_total(ld_1, ld_2), l_engs))
                ...
                grid_res = int(np.sqrt(len(self.grid_xy)))
                ldos_cache_name = f"ldos_topo_{bias_e}V_{self.ldos_height}A_{grid_res}px{self._broadening_cache_suffix()}.npy"
                if exists(ldos_cache_name):
                    ...
                    ld_1, ld_2, init_engs = self._calculate_ldos_at_points_gpu(
                        cp.hstack([self.grid_xy_gpu, z_fixed[:, None]]), t_emin, t_emax,
                        use_energy_decay=self.use_decay_topo, preserve_orbitals=False)
                    target_setp = cp.max(gpu_simpson(self._combine_ldos_total(ld_1, ld_2), init_engs))
